...
namespace DemoApplication.Areas.Admin.Controllers
{
    ...
    public class BookImageController : Controller
    {
        ...
        [HttpGet("{bookId}/image/update", Name = "admin-book-image-update")]
        public async Task<IActionResult> UpdateAsync([FromRoute] int bookId)
        {
            var bookImage = await _dataContext.BookImages.FirstOrDefaultAsync(b => b.Id == bookId);
        ...
        [HttpPost("{bookId}/image/update", Name = "admin-book-image-update")]
        public async Task<IActionResult> UpdateAsync([FromRoute] int bookId, [FromForm] UpdateViewModel model)
        {
            if (!ModelState.IsValid)
            {
                return View(model);
            ...
            var imageNameInSystem = await _fileService.UploadAsync(model.ImageName!, UploadDirectory.Book);

            await _fileService.DeleteAsync(imageNameInSystem, UploadDirectory.Book);

            var bookImage = new UpdateViewModel
            {
                ImageUrl = _fileService.GetFileUrl(imageNameInSystem, UploadDirectory.Book)
            };
